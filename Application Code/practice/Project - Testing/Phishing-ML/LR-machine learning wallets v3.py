...
file_path = 'datasets/ethereum/phishing/transaction_dataset_even.csv'
...
# Updated feature list including engineered features
initial_features = [
...
transaction_data_selected = transaction_data[initial_features].apply(pd.to_numeric, errors='coerce')

# Replace missing values with zero
transaction_data_selected.fillna(0, inplace=True)
...
transaction_data_selected.fillna(0, inplace=True)  # Again, fill any remaining NaNs with 0
transaction_data_selected = transaction_data_selected.clip(upper=1e10)

# Ensure no NaN values remain before SMOTE
if transaction_data_selected.isna().sum().sum() > 0:
    print("NaN values detected. Replacing NaNs with 0...")
    transaction_data_selected.fillna(0, inplace=True)
...
joblib.dump(top_features, os.path.join('results', 'random_forest_phishing_detector_aggregated_features.pkl'))

# Final NaN check before applying SMOTE
if X_train[top_features].isna().sum().sum() > 0:
    print("NaN values detected in top features. Replacing NaNs with 0...")
    X_train[top_features].fillna(0, inplace=True)
...
joblib.dump(rf_model, os.path.join('results', 'random_forest_phishing_detector_aggregated.pkl'))
