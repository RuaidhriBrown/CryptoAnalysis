...
def get_existing_hashes(connection):
    ...
    query = 'SELECT "hash" FROM dev_crypto_tracker."erc20_transaction"'
...
def import_csv_to_db(csv_file_path, connection):
    ...
    with open(csv_file_path, mode='r', encoding='utf-8') as file:
        ...
        total_rows = sum(1 for _ in open(csv_file_path, 'r', encoding='utf-8')) - 1  # Get total number of rows excluding header
        ...
        for row in tqdm(reader, total=total_rows, desc="Importing transactions"):
            ...
            composite_id = f"{row['hash']}_{row['from']}_{row['to']}"
            data = (
                composite_id,
                ...
                row['tokenName'] if row['tokenName'] else '',  # Set default to empty string if missing
                convert_to_int(row['tokenDecimal'], default=0),  # Set default to 0 if missing
            ...
            query = """
                INSERT INTO dev_crypto_tracker."erc20_transaction" (
                    composite_id, block_number, timestamp, hash, nonce, block_hash, 
                    from_address, contract_address, to_address, value, token_name,
                    token_decimal, transaction_index, gas, 
                    gas_price, gas_used, cumulative_gas_used, input, confirmations, address
                ) VALUES (
...
if __name__ == "__main__":
    ...
    csv_file_path = 'combined_er20.csv'
