...
# Replace infinities and extremely large values
transaction_data_selected.replace([np.inf, -np.inf], np.nan, inplace=True)
transaction_data_selected.fillna(0, inplace=True)

# Ensure all values are finite and within a reasonable range
transaction_data_selected = transaction_data_selected.clip(lower=-1e10, upper=1e10)
...
joblib.dump(top_features, '/results/random_forest_moneyLaundering_detector_er20_features.pkl')
...
model_filename = '/results/random_forest_moneyLaundering_detector_er20.pkl'
